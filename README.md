...
You can record a bag when running any tmuxp config by adding the `--record` flag to the `run-adt4` invocation.
This will try and infer the correct topics to record based on the platform ID (e.g., `topaz`, `base_station`), but you can also specify the topic file to use by passing `--record TOPIC_SET_NAME` to `run-adt4`, which should correspond to a file name `{TOPIC_SET_NAME}_topics.txt` in the recording directory.
...
Finally, `run-adt4` supports also running a bag for playing back data. Something like
```
run-adt4 -n hamilton -c topaz spot_bag-bag --sim-time -o ~/hamilton_test --bag /data/datasets/dcist/pennovation/slam_hard/hard_robot_2_hamilton/recorded_data --sim-time
```
will run our initial mapping pipeline against the specified bag.
When doing this, you can also specify a trajectory to use to override the odometry in the bag, i.e.,
```
run-adt4 -n hamilton -c topaz spot_bag-bag --sim-time -o ~/hamilton_test --bag /data/slam_hard/hard_robot_2_hamilton/recorded_data --trajectory /data/slam_hard/hamilton.csv --sim-time
```
