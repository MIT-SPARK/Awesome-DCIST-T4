...
All ROS nodes in `ADT4` load their configuration from
`dcist_launch_system/config/<config_key>/<node_name>.yaml`. The YAML files
under the `<config_key>` directory have a full set of the YAML parameters
...
configurations versus which are system-specific, the system-specific
...
`dcist_launch_system/config_generation/experiment_overrides/<config_key>`
...
For example, consider `base_params/roman_lc_node.yaml`:
```yaml
---
/**:
  ros__parameters:
...
`prior_sesion_maps` and `prior_session_ids` need to be overridden if we have a prior map we
would like to relocalize against.
In `config_generation/experiment_overrides/relocalize/roman_lc_node.yaml`
we specify that these two parameters should be overriden for the `relocalize` parameter setting
to point to the prior map that we will be using.
```yaml
---
/**:
  ros__parameters:
    prior_session_maps: !replace [$(env ADT4_PRIOR_ROMAN_PATH)]
    prior_session_ids: !replace [0]
```

The naming of this file is important -- our compositor will update a `<node
name>.yaml` base param file by searching for `<node name>.yaml` file in the
override directory.
This specifies the values that should be used in the `base_params` file.
The base parameters will always be rendered to a `default` configuration.
...
output parameter files (normally `dcist_launch_system/config`) and where to output
the generated tmux launch files (normally `dcist_launch_system/tmux/`). There
is also a verbose and quiet flag.
You may want to also use the `--clear` flag on occasion to drop unused parameter and
tmux files.
...
./generate_configs.py . ../config ../tmux/
```
...
The generation process is controlled by a file called
...
three parts of this file -- `launch_modes`, `configs`, and `experiments`.

The `launch_modes` section allow you to build up list of reusable windows that should be
included in a tmuxp "launch" file.
Each item under `launch_modes` is a name for a group of windows. A
...
mode don't need to be primitives.

The `configs` follow a similar compositional logic. A configuration key with no children
should be a directory under `experiment_overrides`. A parameter grouping that
does have children will apply all descendant primitive configurations as overrides.

The `experiments` section corresponds to actual tmuxp files that will be rendered,
along with the requisite configurations. Each experiments consists of a specific `launch_mode`
that specifies the windows to include, a list of configurations to use under `config` and `robot_models`
to instantiate the tmuxp file for. This will generate a tmuxp file(s) with the following naming scheme:
```
[<robot_model>-]<experiment_name>-<config_name>
```

For example, if you have an experiment called `mapping` enabled for the robots `spot`, `spot_zed`, and `husky`
and the configurations `default` and `my_awesome_parameters`, you should get the following tmuxp files:
```
spot-mapping-default
spot-mapping-my_awesome_parameters
spot_zed-mapping-default
spot_zed-mapping-my-awesome_parameters
husky-mapping-default
husky-mapping-my-awesome_parameters
```
